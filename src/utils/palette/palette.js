export default {
  ...
  getPalette (buffer) {
    ...
    const { sortedData, total } = this.processData(data)
    const binArr = this.binning(sortedData, total)
    const result = this.mergeBins(binArr)
  ...
  processData (data) {
    ...
    for (let i = 0; i <= length - 4; i = i + 4 * ratio) {
      const hue = this.getHue(data[i], data[i + 1], data[i + 2])
      processedData.push(data[i], data[i + 1], data[i + 2], hue)
    }
    return { sortedData: this.sortByHue(processedData), total: processedData.length / 4 }
  },
  ...
  binning (sortedData, total) {
    ...
    const rec = function (data) {
      ...
      if (that.shouldComplete(data1)) {
        const bin1 = new Bin(data1, total)
        binArr.push(bin1)
      } else {
        rec(data1)
      }
      if (that.shouldComplete(data2)) {
  ...
  mergeBins (binArr) {
    ...
    do {
      ...
      if (binArr[i].trivial || this.getDistance(binArr[i + 1].average, binArr[i].average) <= 36) {
